...
- `features/<feature>/` — one folder per UI feature: `library/`, `playlist/`, `deck/` (NowPlaying.svelte + CueDeck.svelte + Waveform.svelte + backend.ts + nativeBackend.ts), `scan/`, `settings/` (SettingsOverlay.svelte — Audio, Library Sync, Library Health, Now Playing, Advanced tabs), `health/` (LibraryHealth.svelte), `toolbar/`, `track/` (TrackTooltip.svelte + MetadataOverlay.svelte + CuePointOverlay.svelte)
...
**Library health:** `library::health::Health` keeps one report — missing tracks, exact duplicates (shared fingerprint), possible duplicates (music with the same normalised artist and title) and the latest library check — and re-emits it as `library-health` after scans, the analysis pass, metadata edits, path changes and purges. The app never deletes audio files: an unwanted copy is deleted by the operator, then marked missing by a scan and purged with `purge_tracks(ids)`. Dismissals (`health_dismissals` table; the check's in memory) silence the badge only while the finding is unchanged. The playlist engine takes missing ids from the same event: advancement drops them up to the next stop marker, even on a cold cache. The library check runs at launch and every `tuning.library.checkIntervalMin` minutes, never alongside a scan, and reports only. See `docs/library-health.md`.
